...
- **Jonathan Acuña / Simple Tech Skills Agent Config (Apr-2026):** New client onboarding via Metricool. AI Consultant & Trainer ("Doctor AI") connecting TikTok + YouTube. Full config at `docs/agent-config/JONATHAN_ACUNA_AGENT_CONFIG.md`. Includes: system prompt (English-primary, bilingual), knowledge base (products, courses, pricing, cases), guardrails (anti-AI markers, platform adaptation), and a new **Link Magnet / Keyword CTA Automation** section — a pattern for handling "comment keyword X to receive resource Y via DM" flows using the existing `autoPrivateReply` infrastructure with contextual intent detection (vs. ManyChat's exact-match approach). Uses operational gates (campaign active, DM eligibility, confidence level, fallback) to control automation.
